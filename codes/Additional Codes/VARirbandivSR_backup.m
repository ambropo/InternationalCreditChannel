function [RR,Fcomp,yArtificial,srIRF,resids,...
    A0Mats,MTidx,nRot,R2first,relEig] = VARirbandivSR_backup(VAR,VARopt,IV,foIV,signRestMat,...
...
nRot = MTidx;
resids = zeros([size(resid) nDraws]);
...
yArtificial = NaN([size(y_artificial) nDraws]);
RR = NaN(nObs,nDraws);
Fcomp = NaN(nVar*nLag,nVar*nLag,nDraws);
...
disp('-- Done!');
srIRF = permute(cat(1,srIRF{:}),[2 3 4 1]);

% extract impact matrices
A0Mats = squeeze(srIRF(1,:,:,:));
%% Compute the error bands etc
%=========================
RR = RR(:,MTidx>0);
Fcomp = Fcomp(:,:,MTidx>0);
