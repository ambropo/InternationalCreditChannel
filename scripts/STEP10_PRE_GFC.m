...
userLO = '2007m12';
finShockSignOnly = 1;
nDraws=1e4; %reduce number as many draws satisfy restrictions
maxIters = 1e2;
...
for ss = 1:3
    ...
    optfont = FigFontOption(fontSize); optfont.title_weight = 'bold'; FigFont(optfont)
    SaveFigure([folderOut 'US ' sNames{ss} ' pre-GFC'],highQuality)
    clf('reset')
...
for ss = 1:3
    ...
    opt=LegOption; opt.handle = handle; LegSubplot({'Mean','68% C.I.','90% C. I.'},opt);
    optfont = FigFontOption(fontSize); optfont.title_weight = 'bold'; FigFont(optfont)
    SaveFigure([folderOut 'UK ' sNames{ss} ' pre-GFC'],highQuality)
    clf('reset')
