...
% Set endogenous US
vNamesLongUS = {'1yr Rate';'Bond Yield';'Unemployment';'CPI';'Excess Bond Premium'};
vNamesUS      = {'i_1YR';'BondYield';'U';'CPI';'EBP'};
nVarUS   = length(vNamesUS);
vTreatUS = zeros(nVarUS,1);  %0=nothing, 1=log, 2=logdiff, 3=diff

% Set endogenous UK
vNamesLongSOE = {'1yr rate';'CPI';'Exch. Rate ($ per £)';'Corporate Spread';'Unemployment'};
vNamesSOE      = {'i_1YR';'CPI'; 'FX';'CS_i_10YR'; 'U'};
...
for ss = 1:3
    ...
    optfont = FigFontOption(fontSize); optfont.title_weight = 'bold'; FigFont(optfont)
    SaveFigure([folderOut 'US ' sNames{ss} ' with unemployment'],highQuality)
    clf('reset')
...
for ss = 1:3
    ...
    opt=LegOption; opt.handle = handle; LegSubplot({'Mean','68% C.I.','90% C. I.'},opt);
    optfont = FigFontOption(fontSize); optfont.title_weight = 'bold'; FigFont(optfont)
    SaveFigure([folderOut 'UK ' sNames{ss} ' with unemployment'],highQuality)
    clf('reset')
